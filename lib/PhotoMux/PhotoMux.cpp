...
PhotoMux::PhotoMux(const uint8_t selectPins[3], const uint8_t muxPins[4]) {
    ...
    thresholds[FRONT] = 10000000000; // - sample values
    thresholds[LEFT] = 18500000000;
    thresholds[RIGHT] = 3000000000;
    thresholds[BACK] = 60000;
...
int PhotoMux::readSensor(uint8_t muxIndex, uint8_t channel) {
    ...
    int value = analogRead(_muxPins[muxIndex]);
    value = value * value;
    return value;
}
