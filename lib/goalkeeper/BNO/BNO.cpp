...
// Adafruit BNO08x (BNO085) driver — structured to mirror the BNO055 driver.
...
// Start retrieving data from the BNO085 sensor, enable the game rotation vector report
void BNO085::InitializeBNO()
{
    Serial.println("Initializing BNO085...");
    Serial.println("BNO I2C Address: 0x4A");
    Serial.println("Teensy 4.1 SDA: pin 18, SCL: pin 19");

    if (!bno_.begin_I2C(0x4A, &Wire))
    {
        Serial.println("ERROR: BNO085 not detected at address 0x4A!");
        Serial.println("Check:");
        Serial.println("  - I2C wiring (SDA pin 18, SCL pin 19)");
        Serial.println("  - Pull-up resistors on SDA/SCL");
        Serial.println("  - BNO085 address (should be 0x4A, SA0 low)");
        while (1);
    }
    delay(1000);
...
// Retrieve the yaw angle from the BNO085 sensor using quaternion data from the game rotation vector
void BNO085::GetBNOData()
{
    if (bno_.wasReset()) {
        bno_.enableReport(SH2_GAME_ROTATION_VECTOR, 5000);
    }
    ...
    while (bno_.getSensorEvent(&sensor_value_)) {
        if (sensor_value_.sensorId == SH2_GAME_ROTATION_VECTOR) {
            ...
            // Negated to match BNO055 Euler convention (CW positive heading),
            // which is what the PID + motor sign convention was tuned for.
